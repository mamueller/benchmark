...
#        
    def test_change_of_base(self):
        ...
        IA_rc=TensorIndexSet("IA_bc_Br",[bc,Br])
        a,b=symbols("a,b")
        IA_rc[0,0]=a
        IA_rc[1,1]=b
        ...
        #Bc.connect(bc,M)
        #j=Ind("j",Bc) # j is an index w.r.t the new cellar base vectors Bc
        #-j would than be the index w.r.t. the roof base bectors Br
        # x(-j) is the TensorIndexSet with respect to the roof base Br
        # self.assertEqual(x(-j)[0],a*x1)
        # self.assertEqual(x(-j)[2],b*x2)
        # it is interesting to see how a multiplication would look like
        C(i,k)=A(i,j)*B(-j,k)#Question is there a ()=operator
