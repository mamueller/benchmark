...
class Cylinder:    
    ...
    def radialpart(self):
        ...
        for j in range(n):
            ...
            testnumrad((self.Dl(self.Dl(Bpart))),etha,1,0.01)
            # #test eq.145
            # testnumrad_eq(self.Dl(alpha_cyl(r)/sqrt(r)),-alpha**2*alpha_cyl(r)/sqrt(r),etha,1,0.01)
            # #test eq. 145 applied twice
            # testnumrad_eq(self.Dl(self.Dl(alpha_cyl(r)/sqrt(r))),alpha**4*alpha_cyl(r)/sqrt(r),etha,1,0.01)
            Wj=Ajs[j]*1.0/(alpha**4*sqrt(r))*alpha_cyl(r)+Bpart
            #testnumrad((self.Dl(self.Dl(Wj))-Fj),etha,1)
            W=W+Wj
